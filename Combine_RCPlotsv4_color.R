...
setwd("~/Desktop/Papers/C-Q_v2/Code")
...
p1<- p1 + labs(
  ...
  annotate(geom="text", x=0.05, y=110000, label=expression("SAD: C = 0.66 x 1.12 x (755 x Q"^1.45*")"),
           color="#DC3220", fontface="bold",hjust = 0,size=4.5)+
  annotate(geom="text", x=0.05, y=0.04, label=expression("SDC: C = 0.71 x 1.23 x (60.0 x Q"^1.20*")"),
...
p1 <- p1+ 
  ...
  scale_x_log10(sec.axis = dup_axis(),breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x)))
p2<-p2+
  annotate(geom="text", x=0.05, y=110000, label= expression("SAD: C = 0.62 x 1.17 x (436 x Q"^1.48*")"),
           color="#DC3220", fontface="bold",hjust = 0,size=4.5)+
  annotate(geom="text", x=0.05, y=0.04, label=expression("SDC: C = 0.69 x 1.28 x (34.0 x Q"^1.21*")"),
           color="#005AB5", fontface="bold",hjust = 0,size=4.5)
p2<- p2+labs(
...
p3 <- p3+labs(
  ...
  annotate(geom="text", x=0.05, y=110000, label= expression("SAD: C = 0.62 x 1.27 x (124 x Q"^1.51*")"),
           color="#DC3220", fontface="bold",hjust = 0,size=4.5)+
  annotate(geom="text", x=0.05, y=0.04, label=expression("SDC: C = 0.69 x 3.57 x (4.08 x Q"^0.986*")"),
           color="#005AB5", fontface="bold",hjust = 0,size=4.5)
p3 <- p3 +
  scale_y_log10(
