...
library(car)
library(boot)
# SAD Sample data where LF and SF-1 has been rescaled with B1 alpha = 1.777750677; CF = 1.274113507)
C <- c(20.17851768, # rescaled LL 300um to 250um
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.4, y=10000, label="Ln(Concentration)= 6.756 + 1.403 × Ln(Discharge)
       Duan Smearing Correction Adjustment = 1.127",
           color="black")

plot+ theme_bw()+
  annotate(geom="text", x=2.5, y=150000, label="Concentration = 1.127 x 859.5 × Discharge^1.403",
           color="black")
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotate(geom="text", x=0.5, y=1, label="Ln(Concentration)= 6.756 + 1.403 × Ln(Discharge)
    Duan Smearing Correction Adjustment = 1.127",
           color="black")+
theme(
...
plot+ theme_bw()+
  annotate(geom="text", x=2.5, y=60000, label="Concentration = 1.127 x 859.5 × Discharge^1.403",
           color="black")
