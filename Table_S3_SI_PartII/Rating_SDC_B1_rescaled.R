...
library(car)
library(boot)
# SDC Sample data where LF has been rescaled with B1 alpha = 1.766902186; CF = 1.270982514)
C <- c(5.297148617,  # rescaled LL 300um to 250um
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 4.219 + 1.150 × Ln(Discharge)
       Duan Smearing Correction Adjustment = 1.228",
                                               color="black")

plot+ theme_bw()+
  annotate(geom="text", x=7.5, y=15000, label="Concentration = 1.228 × (67.96 × Discharge^1.150)",
           color="black")
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 4.219 + 1.150 × Ln(Discharge)
    With Duan Smearing Correction Adjustment = 1.228",
           color="black")+
  theme(
