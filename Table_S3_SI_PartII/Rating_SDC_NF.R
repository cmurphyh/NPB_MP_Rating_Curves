...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 3.527 + 1.214 × Ln(Discharge)
       Duan Smearing Correction Adjustment = 1.272",
           color="black")

plot+ theme_bw()+
  annotate(geom="text", x=7.5, y=15000, label="Concentration = 1.272 × (34.03 × Discharge^1.214)",
           color="black")
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 3.527 + 1.214 × Ln(Discharge)
    With Duan Smearing Correction Adjustment = 1.272",
           color="black") +
theme(
