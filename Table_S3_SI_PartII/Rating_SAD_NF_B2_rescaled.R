...
#where where LF has been rescaled with B2 alpha = 2.662212467; CF = 1.592893367))
C <- c(8.271190137, # rescaled LL 300um to 250um
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 6.327 + 1.384 × Ln(Discharge)
       Duan Smearing Correction Adjustment = 1.186",
           color="black")

plot+ theme_bw()+
  annotate(geom="text", x=7.5, y=1, label="Concentration = 1.186 x 559.5 × Discharge^1.384",
           color="black")
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotate(geom="text", x=0.5, y=1, label="Ln(Concentration)= 6.327 + 1.384 × Ln(Discharge)
    Duan Smearing Correction Adjustment = 1.186",
           color="black")+
  theme(
...
plot+ theme_bw()+
  annotate(geom="text", x=2.5, y=60000, label="Concentration = 1.186 x 559.5 × Discharge^1.384",
           color="black")
