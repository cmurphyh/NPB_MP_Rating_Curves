...
C <- c(1.516384859, # rescaled LL 300um to 250um
...
100*(meanC-meanyhat)/meanC #this is an underestimate of the mean of the observed data by ~1.6%
...
meanmle<-mean(ymle)
meanmle
100*(meanC-meanmle)/meanC #this is an overestimate of the mean of the observed data by ~-19%
...
meanysmear
100*(meanC-meanysmear)/meanC #this is an overestimate of the mean of the observed data by ~-8.8%
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.4, y=10000, label="Ln(Concentration)= 5.073 + 1.412 × Ln(Discharge)
    Duan Smearing Correction Adjustment = 1.242",
           color="black")

plot+ theme_bw()+
  annotate(geom="text", x=2.5, y=150000, label="Concentration = 1.242 x 159.7 × Discharge^1.412",
           color="black")
...
plot <- ggplot()+ 
  ...
  geom_line(aes(x = Q, y = upr_cor), linetype=2, col = "purple", data = pmodel_cor)+
  geom_point(data = df, aes(x=Q, y=C))+
  ggtitle("MP Concentration-Discharge: SAD (TRWPs)") +
  xlab("Discharge, in cubic meters per second") +
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotate(geom="text", x=0.5, y=0.1, label="Ln(Concentration)= 5.073 + 1.412 × Ln(Discharge)
    Duan Smearing Correction Adjustment = 1.242",
           color="black")+
  theme(
...
plot+ theme_bw()+
  annotate(geom="text", x=2.5, y=60000, label="Concentration = 1.242 x 159.7 × Discharge^1.412",
           color="black")
