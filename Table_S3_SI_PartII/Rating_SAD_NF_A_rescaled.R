...
library(car)
library(boot)
# Load SAD Sample Non-Fiber data [n/m3] paired with Discharge [m3/s]
#where where LF has been rescaled with Kooi et al. 2021 freshwater alpha = 2.64; CF = 1.583433109))
C <- c(8.222067205, # rescaled LL 300um to 250um
       10.31320835, # rescaled LL 300um to 250um
       21.53284462, # rescaled LL 300um to 250um
       29.49994479, # rescaled LL 300um to 250um
       1252.833477, # rescaled LL 300um to 250um
       7077.272727,
       13775.75758,
       11221.46119)
Q <- c(0.0679604,
...
plot(df,xlab = "Stream Discharge (cms)", ylab = "Microplastic (Non-Fiber) Concentration (n/m3)", main = "Arithmetic Plot: C-Q at SAD (Non-Fibers only)")
...
100*(meanC-meanyhat)/meanC #this is an overestimate of the mean of the observed data by ~3%
...
meanmle<-mean(ymle)
meanmle
100*(meanC-meanmle)/meanC #this is an overestimate of the mean of the observed data by ~28%
...
meanysmear
100*(meanC-meanysmear)/meanC #this is an overestimate of the mean of the observed data by ~20%
...
plot <- ggplot()+ 
  ...
  ggtitle("MP Concentration-Discharge: SAD (Non-Fibers)") +
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 6.324 + 1.385 × Ln(Discharge)
       Duan Smearing Correction Adjustment = 1.186",
           color="black")

plot+ theme_bw()+
  annotate(geom="text", x=7.5, y=1, label="Concentration = 1.186 x 557.7 × Discharge^1.385",
           color="black")
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotate(geom="text", x=0.5, y=1, label="Ln(Concentration)= 6.324 + 1.385 × Ln(Discharge)
    Duan Smearing Correction Adjustment = 1.186",
           color="black")+
  theme(
...
plot+ theme_bw()+
  annotate(geom="text", x=2.5, y=60000, label="Concentration = 1.186 x 557.7 × Discharge^1.385",
           color="black")
