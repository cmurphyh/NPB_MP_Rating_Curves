...
C <- c(3.320436402, # rescaled LL 300um to 250um
       3.656612519, # rescaled LL 300um to 250um
       13.51010096, # rescaled LL 300um to 250um
       2.385330955, # rescaled LL 300um to 250um
       2.458900816, # rescaled LL 300um to 250um
       4.506311208, # rescaled LL 300um to 250um
       3.994533576, # rescaled LL 300um to 250um
               1863.333333,
               983.3032491,
               1735.205184,
               16.43192488,
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 3.767 + 1.124 × Ln(Discharge)
       Duan Smearing Correction Adjustment = 1.270",
           color="black")

plot+ theme_bw()+
  annotate(geom="text", x=7.5, y=15000, label="Concentration = 1.270 × (43.25 × Discharge^1.124)",
           color="black")
