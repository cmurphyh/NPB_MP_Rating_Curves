...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 6.078 + 1.479 × Ln(Discharge)
       Duan Smearing Correction Adjustment = 1.170",
           color="black")

plot+ theme_bw()+
  annotate(geom="text", x=7.5, y=1, label="Concentration = 1.170 x 435.98 × Discharge^1.479",
           color="black")
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotate(geom="text", x=0.5, y=1, label="Ln(Concentration)= 6.078 + 1.479 × Ln(Discharge)
    Duan Smearing Correction Adjustment = 1.170",
           color="black")+
  theme(
...
plot+ theme_bw()+
  annotate(geom="text", x=2.5, y=60000, label="Concentration = 1.170 x 435.98 × Discharge^1.479",
           color="black")
