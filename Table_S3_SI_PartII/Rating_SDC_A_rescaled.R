...
library(car)
library(boot)
# SDC Sample data where LF has been rescaled with Kooi et al. 2021 freshwater alpha = 2.64; CF = 1.583433109)
C <- c(6.59936735,  # rescaled LL 300um to 250um
       7.328211154, # rescaled LL 300um to 250um
       19.71078559, # rescaled LL 300um to 250um  
       5.643343967, # rescaled LL 300um to 250um  
       3.564581049, # rescaled LL 300um to 250um  
       7.244953378, # rescaled LL 300um to 250um  
       8.255369389, # rescaled LL 300um to 250um  
                   3407.222222,
                   1405.415162,
                   2632.829374,
                   1302.367942,
...
mean(Q)
###########################
#Comparision to determine Final Model
###########################
...
meanmle<-mean(ymle)
meanmle
100*(meanC-meanmle)/meanC #this is an overestimate of the mean of the observed data by ~-5%
...
meanysmear
100*(meanC-meanysmear)/meanC #this is an overestimate of the mean of the observed data by ~-2%
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 4.334 + 1.107 × Ln(Discharge)
       Duan Smearing Correction Adjustment = 1.227",
                                               color="black")

plot+ theme_bw()+
  annotate(geom="text", x=7.5, y=15000, label="Concentration = 1.227 × (76.21 × Discharge^1.107)",
           color="black")
...
plot <- ggplot()+ 
  ...
  geom_line(aes(x = Q, y = upr_cor), linetype=2, col = "purple", data = pmodel_cor)+
  geom_point(data = df, aes(x=Q, y=C))+
  ggtitle("MP Concentration-Discharge: SDC (all Microplastics)") +
  xlab("Discharge, in cubic meters per second") +
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 4.334 + 1.107 × Ln(Discharge)
    With Duan Smearing Correction Adjustment = 1.227",
           color="black")+
  theme(
