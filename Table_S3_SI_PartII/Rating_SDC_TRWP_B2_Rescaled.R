...
library(car)
library(boot)
# Load SDC Sample TRWP data [n/m3] paired with Discharge [m3/s]
#where where LF has been rescaled with B2 alpha = 2.433978673; CF = 1.499146674))
C <- c(0.589441681, # rescaled LL 300um to 250um
...
mean(Q)
###########################
#Dont need to run comparison again.  Skip to Final Model
###########################
...
plot(df,xlab = "Stream Discharge (cms)", ylab = "Microplastic Concentration (n/m3)", main = "Arithmetic Plot: C-Q at SDC")
...
plot(df, xlab="Discharge, in cubic meters per second",
     ylab="Suspected TRWPs per cubic meter", main="Untransformed C-Q")
...
plot(df, xlab="Discharge, in cubic meters per second",
     ylab="Suspected TRWPs per cubic meter", main="Natural Log Transformed C only", log = 'y')
#Model and look at significance and residual plots
model <- lm(log(C) ~ Q, data = df)
summary(model)
#plot(model) Using below plots instead
...
#Plot data in log-log space
plot(df, xlab="Discharge, in cubic meters per second",
     ylab="Suspected TRWPs per cubic meter", main="Natural Log Transformed C and Q", log = 'xy')
#Model and look at significance and residual plots
model <- lm(log(C) ~ log(Q), data = df)
summary(model)
#plot(model) #Using below plots instead
...
100*(meanC-meanyhat)/meanC #this is an underestimate of the mean of the observed data by ~75%
...
meanmle<-mean(ymle)
meanmle
100*(meanC-meanmle)/meanC #this is an overestimate of the mean of the observed data by ~-44%
...
meanysmear
100*(meanC-meanysmear)/meanC #this is an overestimate of the mean of the observed data by ~12%
...
plot <- ggplot()+ 
  ...
  ggtitle("MP Concentration-Discharge: SDC") +
  xlab("Discharge, in cubic meters per second") +
  ylab("Total Suspected TRWPs per cubic meter")
plot
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 1.6176 + 0.9066 × Ln(Discharge)
       Duan Smearing Correction Adjustment = 3.211",
                                               color="black")

plot+ theme_bw()+
  annotate(geom="text", x=7.5, y=15000, label="Concentration = 3.211 × (5.041 × Discharge^0.9066)",
           color="black")
...
plot <- ggplot()+ 
  ...
  geom_line(aes(x = Q, y = upr_cor), linetype=2, col = "purple", data = pmodel_cor)+
  geom_point(data = df, aes(x=Q, y=C))+
  ggtitle("MP Concentration-Discharge: SDC (TRWPs)") +
  xlab("Discharge, in cubic meters per second") +
  ylab("TRWPs, number per cubic meter")
plot
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
                       labels = trans_format("log10", math_format(10^.x))) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotate(geom="text", x=0.5, y=15000, label="Ln(Concentration)= 1.6176 + 0.9066 × Ln(Discharge)
       Duan Smearing Correction Adjustment = 3.211",
           color="black") +
theme(
  ...
  annotate("text", x = 17.5, y = 0.02, label = "c", size = 6.5, fontface = "bold") 

plot+ theme_bw()+
  annotate(geom="text", x=7.5, y=15000, label="Concentration = 3.211 × (5.041 × Discharge^0.9066)", color="black")
