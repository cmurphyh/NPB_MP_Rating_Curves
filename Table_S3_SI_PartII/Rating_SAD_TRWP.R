...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.4, y=10000, label="Ln(Concentration)= 4.824 + 1.507 × Ln(Discharge)
    Duan Smearing Correction Adjustment = 1.265",
           color="black")

plot+ theme_bw()+
  annotate(geom="text", x=2.5, y=150000, label="Concentration = 1.265 x 4.824 × Discharge^1.507",
           color="black")
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotate(geom="text", x=0.5, y=0.1, label="Ln(Concentration)= 4.824 + 1.507 × Ln(Discharge)
    Duan Smearing Correction Adjustment = 1.265",
           color="black")+
  theme(
...
plot+ theme_bw()+
  annotate(geom="text", x=2.5, y=60000, label="Concentration = 1.265 x 124.4619 × Discharge^1.507",
           color="black")
