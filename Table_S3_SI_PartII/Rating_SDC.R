...
meanmle<-mean(ymle)
meanmle
100*(meanC-meanmle)/meanC #this is an overestimate of the mean of the observed data by ~-8%
...
meanysmear
100*(meanC-meanysmear)/meanC #this is an overestimate of the mean of the observed data by ~-4%
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw()+
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 4.094 + 1.197 × Ln(Discharge)
       Duan Smearing Correction Adjustment = 1.232",
                                               color="black")

plot+ theme_bw()+
  annotate(geom="text", x=7.5, y=15000, label="Concentration = 1.232 × (59.96 × Discharge^1.197)",
           color="black")
#+
  #annotate(geom="text", x=7.5, y=17000, label="Ln(Concentration)= 4.09375 + 1.19679 × Ln(Discharge)",
  #         color="gray30")
...
plot    +scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotate(geom="text", x=0.5, y=8000, label="Ln(Concentration)= 4.094 + 1.197 × Ln(Discharge)
    With Duan Smearing Correction Adjustment = 1.232",
           color="black")+
  theme(
